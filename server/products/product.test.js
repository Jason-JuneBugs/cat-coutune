...
describe("GIVEN that the GET /products route exist", () => {
  ...
  test("WHEN there are products THEN return status 200 and an array of products", async () => {
    ...
    const expectedResponseData = {
      products: await productRepository.getProducts(defaultLimit,defaultPage),
      currentPage: 1,
      totalPages: Math.ceil(parseInt(totalProducts.length) / defaultLimit),//totalProducts is a array!
  ...
  describe("WHEN the client sends a request for a specific page of products", () => {
    ...
    test(
      "WHEN the page query parameter is not valid as per the API spec THEN return status 400 and an appropriate error message",async ()=>{
        const response = await request(app)
